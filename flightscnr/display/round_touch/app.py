...
class RoundTouchDisplay:
    def __init__(self):
        ...
        requested_w = theme.WIDTH
        requested_h = theme.HEIGHT
        self._display = video.init_display(requested_w, requested_h, fullscreen)
        dw, dh = self._display.get_size()
        if (dw, dh) != (requested_w, requested_h):
            logger.info(
            ...
            theme.set_framebuffer_dimensions(dw, dh)
            map_bg.invalidate()
            if self._display.get_size() != (theme.WIDTH, theme.HEIGHT):
                pygame.display.quit()
                self._display = video.init_display(theme.WIDTH, theme.HEIGHT, fullscreen)
    ...
    def _apply_brightness(self):
        ...
        # should apply uniformly across all screens, radar included.
            ...
            and off_hours.prefs().get("force_clock")
            and self.screen
    ...
    def run(self):
        logger.info(
            "Touch display starting (%dx%d framebuffer, rotation=%d°, visible radius=%d, round=%s)",
            theme.WIDTH,
            theme.HEIGHT,
