...
def prewarm_frame_layer(flights) -> None:
    ...
    with _layer_lock:
        ...
        backdrop = _backdrop
        if backdrop is None or backdrop.get_size() != (theme.WIDTH, theme.HEIGHT):
            return
        _rebuild_counts["layer"] += 1
        build = _layer_spare
        _layer_spare = None
        if build is None or build.get_size() != (theme.WIDTH, theme.HEIGHT):
            build = pygame.Surface((theme.WIDTH, theme.HEIGHT))
        ...
        old = _frame_layer
        _frame_layer = build
        _frame_layer_key = ((theme.WIDTH, theme.HEIGHT), _backdrop_gen)
        _frame_layer_at = time.time()
        _frame_layer_gen += 1
        if old is not None and old.get_size() == (theme.WIDTH, theme.HEIGHT):
            _layer_spare = old
...
def _ensure_backdrop(*, calibrate: bool, pan_mode: bool, pan_offset) -> pygame.Surface | None:
    ...
    if key is None:
        return None
    if _backdrop is not None and _backdrop_key == key and _backdrop.get_size() == (theme.WIDTH, theme.HEIGHT):
        return _backdrop

    _rebuild_counts["backdrop"] += 1
    surf = pygame.Surface((theme.WIDTH, theme.HEIGHT))
...
def _frame_layer_fresh(key) -> bool:
    return (
        ...
        and _frame_layer.get_size() == (theme.WIDTH, theme.HEIGHT)
...
def _ensure_frame_layer(backdrop, flights, offset) -> pygame.Surface | None:
    ...
    key = ((theme.WIDTH, theme.HEIGHT), _backdrop_gen)
    ...
    if not _layer_lock.acquire(blocking=False):
        if _frame_layer is not None and _frame_layer.get_size() == (theme.WIDTH, theme.HEIGHT):
    ...
    try:
        if _frame_layer_fresh(key):
            return _frame_layer
        _rebuild_counts["layer"] += 1
        if _frame_layer is None or _frame_layer.get_size() != (theme.WIDTH, theme.HEIGHT):
            _frame_layer = pygame.Surface((theme.WIDTH, theme.HEIGHT))
        _t = time.perf_counter()
